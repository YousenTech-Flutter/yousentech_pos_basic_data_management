...
import 'category_sale_price_service.dart';

class CategorySalePriceController extends GetxController {
  final CategorySalePriceService _categorySalePrice = CategorySalePriceService.getInstance();

  var isLoading = false.obs;
  var hideMainScreen = false.obs;
  List<CategorySalePrice> categorySalePriceList = [];
  RxList<CategorySalePrice> searchResults = RxList<CategorySalePrice>();
  CategorySalePrice? object;

  @override
  Future<void> onInit() async {
    super.onInit();
    await _categorySalePricesData();
  }

  _categorySalePricesData() async {
    var result = await displayCategorySalePriceList();
    categorySalePriceList = result.data;
    update();
  }
...
// # ===================================================== [ DISPLAY ACCOUNT JOURNAL LIST ] =====================================================
  ...
  Future<dynamic> displayCategorySalePriceList() async {
    isLoading.value = true;
    dynamic result = await _categorySalePrice.index();
...
// ============================================ [ SEARCH PRODUCT UNIT ] ===============================================
  ...
  Future<void> search(String query) async {
    if (categorySalePriceList.isNotEmpty) {
      searchResults.clear();
      var result = await _categorySalePrice.search(query);
      if (result is List) {
        searchResults.addAll(result as List<CategorySalePrice>);
      }
      update();
    }
  }
